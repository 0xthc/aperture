...
def _build_founder(conn, row) -> dict:
    ...
    return {
        ...
        "entityType": row["entity_type"] or "individual",
        "sources": sources,
        "tags": tags,
        ...
        "twitter_handle": row["twitter_handle"] if "twitter_handle" in row.keys() else "",
        "twitter_followers": row["twitter_followers"] if "twitter_followers" in row.keys() else 0,
        "twitter_engagement_rate": row["twitter_engagement_rate"] if "twitter_engagement_rate" in row.keys() else 0,
        "linkedin_url": row["linkedin_url"] if "linkedin_url" in row.keys() else "",
        "linkedin_summary": row["linkedin_summary"] if "linkedin_summary" in row.keys() else "",
        "is_serial_founder": bool(row["is_serial_founder"]) if "is_serial_founder" in row.keys() else False,
